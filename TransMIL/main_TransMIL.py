...
def arg_parse():
    ...
    parser.add_argument("--graph_mode", type=str, default="rag", choices=['embedding', 'rag', 'knn', 'krag'], help="Change type of graph used for training here")
...
def main(args):
    ...
    run_results_folder = f"TransMIL_{args.convolution}_PE_{args.encoding_size}_{args.embedding_net}_{args.dataset_name}_{args.seed}_{args.heads}_{args.pooling_ratio}_{args.learning_rate}"
    ...
    for fold_idx, (train_fold, test_fold) in enumerate(zip(training_folds, testing_folds)):
        ...
        graph_net = TransMIL(num_features=1000, n_classes=2)
